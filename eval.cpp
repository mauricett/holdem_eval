...
constexpr int player = 6; //max values für player und cardcnt
...
void test(int (&board)[10], int(&hands)[6 * 2 * 2], int (&merged)[cardcnt * player], const int player)
{
	sort(begin(board), begin(board) + 5);
	for (int i = 0; i < player; ++i)
	{
	...
	{
		//dif[6 * p] = merged[7 * p + 1] - merged[7 * p]; //das erste dif-element jedes spielers
		for (int i = 0; i < (cardcnt - 1); ++i)
		{
			dif[i + 6 * p] = merged[7 * p + i + 1] - merged[7 * p + i];
			//if ((dif[i - 1] == 0) && (dif[i] != 0)) { ++counter[p]; }
		}
	}
}
...
void find_winner(int(&board)[10], int(&hands)[6 * 2 * 2], int(&merged)[cardcnt * player], const int player)
{
	...
	//ersten rechnungen sind außerhalb der for-schleife, aber gehören von logik her dazu
	for (int p = 0; p < player; ++p)
	{
		...
		for (int i = 1; i < (cardcnt - 1); ++i)
		{
			...
			case 1:
			{
				...
				case 1:
					...
					if (straight[p] >= 4) { straight_poiner[p] = &(merged[7 * p + i + 1]); } 	//check alle straights und straight flushs, bis auf wheelie. pointer auf letzte straight-karte!
					break;
				default:
					max_tmp[p] = 0;
					break;
